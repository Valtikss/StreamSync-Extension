...
# 🎵 StreamSync - Browser Extension (Chrome & Firefox)
...
1. Download or clone this repository:
   ...
   ```
2. Build the unpacked extension(s):
   ```bash
...
3. Load the extension in your browser:
   - **Chrome / Chromium / Edge:** open `chrome://extensions/`, enable **Developer mode** (top-right), click **Load unpacked**, select `dist/chrome/`.
   - **Firefox:** open `about:debugging#/runtime/this-firefox`, click **Load Temporary Add-on…**, select `dist/firefox/manifest.json`.
4. The StreamSync icon should appear in your toolbar - pin it for easy access.

> The `dist/` folder is git-ignored. Re-run `npm run build` after any code change.
> Pre-built zips are published on the [Releases page](../../releases) — download and load unpacked (Chrome) or via `about:debugging` (Firefox, temporary).
...
1. Open the StreamSync extension popup
...
> ⚠️ The Redirect URI is **different on Chrome and Firefox** (Chrome uses `https://<id>.chromiumapp.org/`, Firefox uses `https://<id>.extensions.allizom.org/`). If you use both browsers, register **both URIs** in your Spotify app settings.
...
```
├── manifest.chrome.json  # Manifest V3 for Chrome / Chromium
├── manifest.firefox.json # Manifest V3 for Firefox (browser_specific_settings.gecko)
├── build.mjs             # Build script - assembles dist/chrome/ and dist/firefox/
├── package.json          # npm scripts (build, build:chrome, build:firefox, clean)
...
└── dist/                 # Generated by `npm run build` (git-ignored)
    ├── chrome/           # Unpacked Chrome build
    └── firefox/          # Unpacked Firefox build
```

The two manifests share the exact same source code; only the manifest differs:
- Chrome keeps the `key` field (stable extension ID for dev) and uses `background.service_worker`.
- Firefox uses `browser_specific_settings.gecko` (id + `strict_min_version: "140.0"`, `data_collection_permissions`) and `background.scripts` (Firefox MV3 doesn't support `service_worker` outside a flag).
